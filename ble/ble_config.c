...
{
    .device_name               = (uint8_t*)"Sierra BLE Test", /**< Local device name (NULL terminated) */
...
{
/*  { buf_size, buf_count } */
    { 64,       6   },      /* Small Buffer Pool */
    { 360,      6   },      /* Medium Buffer Pool (used for HCI & RFCOMM control messages, min recommended size is 360) */
    { 360,      16  },      /* Large Buffer Pool  (used for HCI ACL messages) */
    { 360,      4   },      /* Extra Large Buffer Pool - Used for avdt media packets and miscellaneous (if not needed, set buf_count to 0) */
};
#endif
