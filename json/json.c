...
/* assemble command in print_status(3) style */
{
				     ...
				     /*@null@*/ const char **end)
{
	...
	*end = NULL;		/* give it a well-defined value on parse failure */
	...
	if (!cursor->nodefault) {
		...
		switch (cursor->type) {
		case t_integer:
		    memmove(lptr, &cursor->dflt.integer, sizeof(int));
		    break;
		case t_uinteger:
		    memmove(lptr, &cursor->dflt.uinteger, sizeof(unsigned int));
		    break;
		case t_time:
		case t_real:
		    memmove(lptr, &cursor->dflt.real, sizeof(double));
		    break;
		case t_string:
		    if (parent != NULL
		    ...
		    break;
		case t_boolean:
		    memmove(lptr, &cursor->dflt.boolean, sizeof(bool));
		    break;
		case t_character:
	...
	case post_val:
		...
		return JSON_ERR_BADENUM;
	      foundit:
		(void)snprint_status(valbuf, sizeof(valbuf), "%d", mp->value);
	    }
	    lptr = json_target_address(cursor, parent, offset);
	    if (lptr != NULL)
		switch (cursor->type) {
		case t_integer:
		    {
			int tmp = atoi(valbuf);
			memmove(lptr, &tmp, sizeof(int));
		    }
		    break;
		case t_uinteger:
		    {
			unsigned int tmp = (unsigned int)atoi(valbuf);
			memmove(lptr, &tmp, sizeof(unsigned int));
		    }
		    break;
		case t_time:
		    {
			double tmp = iso8601_to_unix(valbuf);
			memmove(lptr, &tmp, sizeof(double));
		    }
		    break;
		case t_real:
		    {
