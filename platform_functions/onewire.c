...
/** @file onewire.c
 *
 *  Derived from the ST reference for One Wire operation
 *
 *  Provide a Onewire interface for iMatrix Things
 *
...
void imx_OneWire_Init(imx_OneWire_t* OneWireStruct, wiced_gpio_t GPIO_Pin ) {
    ...
    /* Init GPIO pin */
    wiced_gpio_init( GPIO_Pin, OUTPUT_OPEN_DRAIN_PULL_UP );
...
uint8_t imx_OneWire_Reset(imx_OneWire_t* OneWireStruct) {
    ...
    /* Check bit value */
    i = wiced_gpio_input_get(OneWireStruct->GPIO_Pin);
...
uint8_t imx_OneWire_ReadBit(imx_OneWire_t* OneWireStruct) {
    ...
    /* Read line value */
    if (wiced_gpio_input_get(OneWireStruct->GPIO_Pin)) {
        /* Bit is HIGH */
        bit = 1;
    }
...
void imx_OneWire_WriteByte(imx_OneWire_t* OneWireStruct, uint8_t byte) {
    ...
    while (i--) {
        /* LSB bit is first */
        imx_OneWire_WriteBit(OneWireStruct, byte & 0x01);
        byte >>= 1;
    }
}
...
uint8_t imx_OneWire_ReadByte(imx_OneWire_t* OneWireStruct) {
    ...
    while (i--) {
        byte >>= 1;
        byte |= (imx_OneWire_ReadBit(OneWireStruct) << 7);
    }
...
uint8_t imx_OneWire_First(imx_OneWire_t* OneWireStruct) {
    /* Reset search values */
    imx_OneWire_ResetSearch(OneWireStruct);

    /* Start with searching */
    return imx_OneWire_Search(OneWireStruct, ONEWIRE_CMD_SEARCHROM);
}
...
uint8_t imx_OneWire_Search(imx_OneWire_t* OneWireStruct, uint8_t command) {
    ...
    if (!OneWireStruct->LastDeviceFlag) {
        // 1-Wire reset
        if (imx_OneWire_Reset(OneWireStruct)) {
            /* Reset the search */
            OneWireStruct->LastDiscrepancy = 0;
            OneWireStruct->LastDeviceFlag = 0;
            OneWireStruct->LastFamilyDiscrepancy = 0;
        ...
        imx_OneWire_WriteByte(OneWireStruct, command);

        // loop to do the search
        do {
            // read a bit and its complement
            id_bit = imx_OneWire_ReadBit(OneWireStruct);
            cmp_id_bit = imx_OneWire_ReadBit(OneWireStruct);
            ...
            if ((id_bit == 1) && (cmp_id_bit == 1)) {
                ...
                imx_OneWire_WriteBit(OneWireStruct, search_direction);
...
int imx_OneWire_Verify(imx_OneWire_t* OneWireStruct) {
    ...
    if (imx_OneWire_Search(OneWireStruct, ONEWIRE_CMD_SEARCHROM)) {
...
void imx_OneWire_TargetSetup(imx_OneWire_t* OneWireStruct, uint8_t family_code) {
   uint8_t i;
...
void imx_OneWire_FamilySkipSetup(imx_OneWire_t* OneWireStruct) {
...
void imx_OneWire_Select(imx_OneWire_t* OneWireStruct, uint8_t* addr) {
    uint8_t i;
    imx_OneWire_WriteByte(OneWireStruct, ONEWIRE_CMD_MATCHROM);

    for (i = 0; i < 8; i++) {
        imx_OneWire_WriteByte(OneWireStruct, *(addr + i));
    }
}
...
void imx_OneWire_SelectWithPointer(imx_OneWire_t* OneWireStruct, uint8_t *ROM) {
    ...
    for (i = 0; i < 8; i++) {
        imx_OneWire_WriteByte(OneWireStruct, *(ROM + i));
    }
}
