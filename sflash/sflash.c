...
extern app_header_t apps_lut[ FULL_IMAGE ];// Look Up Table descriptions of the 8 entries that may be used as bootable images by the bootloader.
...
void get_apps_lut_if_needed(void)
{
	...
	if ( apps_lut_requires_initialization ) {
        ...
        /*
         * Verify LUT against valid
         */
        if( memcmp( &apps_lut, &valid_lut, sizeof( apps_lut ) ) == 0 )
        	return;
        else {
        	imx_printf( "LUT invalid, loading with default values\r\n" );
        	memcpy( &apps_lut, &valid_lut, sizeof( apps_lut ) );
