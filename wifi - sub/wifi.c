...
extern dcb_t dcb;
// !@# extern dcb_t dcb;
extern Fixture_Status_t fs;
extern Fixture_Config_t fc;
...
uint16_t wifi_init(void)
{
	...
	if( device_config.AP_setup_mode == true ) {
	    ...
	    /*
	     * Enable CLI during Setup mode
	     */
        device_config.cli_enabled = true;
        device_config.telnet_enabled = true;
        device_config.ssh_enabled = true;
        imatrix_save_config();
	    ...
	    if( ( device_config.st_security_mode == SECURITY_8021X_PEAP ) || ( device_config.st_security_mode == SECURITY_8021X_EAP_TLS ) ) {
	    		wiced_result = join_ent( device_config.st_ssid, device_config.st_eap_mode, WICED_SECURITY_WPA2_MIXED_ENT, (uint8_t *) device_config.device_serial_number, (uint8_t *) device_config.password );
	    		// malloc_info();
	    } else {
	        set_wifi_st_ssid( device_config.st_ssid, device_config.st_wpa, device_config.st_security_mode, device_config.st_eap_mode );	// What is stored in the configuration
	     ...
	     */
	    /*
	     * Restrict Access based on settings
	     */
	    if( ( fc.locked_down == true ) /* && ( ( device_config.st_security_mode == SECURITY_8021X_PEAP ) || ( device_config.st_security_mode == SECURITY_8021X_EAP_TLS ) ) */ ) {
	        device_config.cli_enabled = false;
	        device_config.telnet_enabled = false;
	        device_config.ssh_enabled = false;
            device_config.imatrix_enabled = false;      // Enable once secure CoAP in place
	        imatrix_save_config();
	    } else {    // Revert if mode has changed
            device_config.cli_enabled = true;
            device_config.telnet_enabled = true;
            device_config.ssh_enabled = true;
            device_config.imatrix_enabled = true;
            imatrix_save_config();
	    }
	    /*
	     * Set up remaining items so system can operate
	     */
...
uint16_t check_valid_ip_address(void)
{
	if( dcb.wifi_up == true  && device_config.AP_setup_mode == false) {
		...
		if( ( dcb.gw_ip.ip.v4 == 0 ) || ( dcb.my_ip.ip.v4 == 0  ) ) {
			print_status( "0.0.0.0 Zero IP Address - failing network\r\n" );
	    	dcb.wifi_up = false;
	        return false;
		}
	}
...
{
	set_wifi_st_ssid( DEFAULT_ST_SSID, DEFAULT_ST_KEY, DEFAULT_ST_SECURITY, 0 );
}
...
void set_wifi_st_ssid( char *ssid, char *passphrase, wiced_security_t security, uint16_t eap_mode )
{
    ...
    device_config.st_eap_mode = eap_mode;
    imatrix_save_config();
